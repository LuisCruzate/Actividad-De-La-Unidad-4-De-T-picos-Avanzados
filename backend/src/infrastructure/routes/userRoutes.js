 ...
 * Obtener perfil del usuario actual (by user-id header)
 */
router.get("/profile", async (req, res) => {
  try {
    const userId = req.headers["user-id"];
    if (!userId) {
      return res.status(401).json({ message: "user-id header requerido" });
    }
 ...
 * Actualizar perfil del usuario actual (by user-id header)
 */
router.put("/profile", async (req, res) => {
  try {
    ...
    const user = await actualizarPerfilUsuario(userId, req.body);
    res.json({
 ...
 * Subir avatar para el usuario actual (by user-id header)
 */
router.post("/profile/upload-avatar", upload.single("avatar"), async (req, res) => {
  try {
    const userId = req.headers["user-id"];
    if (!userId) {
      return res.status(401).json({ message: "user-id header requerido" });
    ...
    const user = await actualizarPerfilUsuario(userId, {
      avatar: avatarPath
    });
